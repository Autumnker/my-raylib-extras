...
> **Note:** `rmouse` is C++ only (uses namespaces, classes).
> You also need to link against raylib.
...
Place raylib next to the `rmouse` folder:

```
your-project/
├── raylib-5.5_win64_mingw-w64/   ← NOT in this repo (see .gitignore)
│   ├── include/
│   └── lib/
└── rmouse/
    ├── src/
...
Then build — choose one of the following:
...
See [`build.bat`](build.bat) (builds the bundled example).
...
> `CMakeLists.txt` is optional — it only builds the example in `examples/`. rmouse itself is header-only and needs no compilation.
...
See [`examples/main.cpp`](examples/main.cpp) for a runnable demo.
